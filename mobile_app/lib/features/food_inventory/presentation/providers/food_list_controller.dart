...
class FoodListController extends StateNotifier<FoodListState> {
  ...
  Future<void> deleteFood(String id) async {
    try {
      ...
      await NotificationService.instance.cancelProductReminders(id);
      await loadItems();
      _ref.read(foodStatsControllerProvider.notifier).loadStats();
    } catch (e) {
      rethrow;
  ...
  Future<void> consumeFood(String id, double quantity) async {
    try {
      ...
      final repo = _ref.read(foodRepositoryProvider);
      final item = await repo.getFoodItemById(id);
  ...
  Future<void> extendExpiry(String id, DateTime newExpiryDate) async {
    try {
      final repo = _ref.read(foodRepositoryProvider);
      final item = await repo.getFoodItemById(id);
      if (item == null) return;
      ...
      await repo.updateFoodItem(updated);
      if (updated.reminderEnabled && updated.expiryDate != null) {
        await NotificationService.instance.scheduleProductReminders(
          item: updated,
          reminderDaysBefore: updated.reminderDaysBefore,
          customReminderDate: updated.reminderDate,
        );
      }
      await loadItems();
      _ref.read(foodStatsControllerProvider.notifier).loadStats();
  ...
  Future<void> dismissProductReminder(String id) async {
    try {
      final repo = _ref.read(foodRepositoryProvider);
      final item = await repo.getFoodItemById(id);
      if (item == null) return;
      ...
      await repo.updateFoodItem(updated);
      await repo.deleteRemindersForProduct(id);
      await NotificationService.instance.cancelProductReminders(id);
      await loadItems();
      _ref.read(foodStatsControllerProvider.notifier).loadStats();
    } catch (e) {
